...
import type { Company, ProductInventory } from "@/lib/data";
import type { Newsletter, Invoice } from "@/lib/mock-data";
...
function getRate(key: string, units: number): number {
  ...
  if (key === "smartFeed") return 0;
  if (key === "smartDelivery") return 0;
  return 0;
}
...
export function CompanyDetailClient({
  ...
  return (
    <div>
      ...
      <PageHeader
        title={company.name}
        subtitle={[company.plan, company.integration, company.owner ? `Owner: ${company.owner}` : ""].filter(Boolean).join(" · ")}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <SummaryCard title="Customer Type" value={company.customerType.join(", ") || "—"} />
      </div>
